...
import {
  ...
  ISnakePosition,
  ISnakeDirection
} from "src/app/inferfaces/games/snake";
...
export class SnakeCanvasComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges) {
    const isPlaying: boolean = changes.isPlaying
      ? changes.isPlaying.currentValue
      : undefined;
    if (isPlaying) {
      this.resetSnake();
      this.scores = 0;
  ...
  public move() {
    ...
    if (
      ...
      this.amountCellsInSnake(this.snakePositions[0]) > 1
    ) {
      this.stop.emit();
    }
  ...
  public isCellOutOfBoard(position: ISnakePosition) {
    return (
      position.x < 0 ||
      position.y < 0 ||
      position.x >= this.snakeGameConfig.boardSize ||
      position.y >= this.snakeGameConfig.boardSize
    );
  }
  ...
  public amountCellsInSnake(position: any) {
    return this.snakePositions.filter(
  ...
  public isTargetNewHead() {
    return (
      this.snakePositions[0].x + this.snakeDirection.move.x ===
        this.targetCell.x &&
      this.snakePositions[0].y + this.snakeDirection.move.y ===
        this.targetCell.y
    );
  }
