...
export class UserEffects {
    ...
    loadUsersList$: Observable<Action> = this.actions$.pipe(
        ...
        switchMap((loadRequest) =>
            this.httpUserService.getUsersList(loadRequest).pipe(
                switchMap((response) => {
                    return [
                        new UserActions.LoadUsersListSuccessAction(response.data)
                    ];
                })
            )
        )
    ...
    updateUser$: Observable<Action> = this.actions$.pipe(
        ...
        switchMap((updateRequest) =>
            this.httpUserService.updateUser(updateRequest.user, updateRequest.file).pipe(
                switchMap((response) => {
                    return [
                        new RouterActions.Go({
                            path: ['/administration/users/list']
    ...
    changeSearchValue$: Observable<Action> = this.actions$.pipe(
        ofType<UserActions.ChangeSearchValueAction>(UserActions.CHANGE_SEARCH_VALUE),
        map(action => action.payload),
        switchMap((value) =>
